...
contract BreedingRouter is Ownable, Pausable, ReentrancyGuard {
    ...
    modifier whenCanBuy(uint256 amount) {
        require(TotalNFTMinted + amount <= TotalNFT, "can't buy");
        _;
    }
    constructor(
    ...
    function buy() public payable whenCanBuy(1) returns (uint256)  {
    ...
    function _rand(uint256 index) internal view returns (uint256) {
        ...
        //here is the logic of it, we have 32 NFT, each with x amount
        // we cannot random with 32, or we may encounter some txn that require re-random over and over again, after some nft has ran out of quantity
